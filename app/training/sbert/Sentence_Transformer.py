...
class custom_SentenceTransformer(nn.Sequential):
    """
    ...
    def fit(self,
            ...
            validation_objectives: Iterable[DataLoader],
        ...
        runn = wandb.init(
            project="Entity Search Model",  # set the wandb project where this run will be logged
        ...
        validation_dataloader = validation_objectives[0]
            ...
            for step in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                for train_idx in range(num_train_objectives):
                    ...
                    if  step % evaluation_steps == 0:
                        ...
                        loss_model.eval()
                        with torch.no_grad():
                            for _ in trange(len(validation_dataloader), desc="Validation Iteration",
                                            smoothing=0.05):
                                validation_data_iterator = iter(validation_dataloader)
                                val_data = next(validation_data_iterator)
                                val_features, val_labels = val_data

                                val_labels = val_labels.to(self._target_device)  # goes to gpu
                                val_features = list(map(lambda batch: batch_to_device(batch, self._target_device),
                            ...
                            wandb.log({"Train_loss": final_train_loss, 'Validation_loss': final_validation_loss,
                                       "Train_rmse": train_mse_metric,
                                       "Validation_rmse": val_mse_metric})  # WANDB LOGGING

                            # print(f"Epoch:{epoch} | Step:{step} | Train_loss: {final_train_loss} | Val_loss: {final_validation_loss}")

                            # wandb.log({"Train_loss": final_train_loss})          # WANDB LOGGING
                        loss_model.train()
