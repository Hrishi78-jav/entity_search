...
with open(DATA_DIR+'/query_positive_map.pkl', 'rb') as f:
    dq = pickle.load(f)

print('Creating sentence pairs')
sp = []
for word, val in tqdm(hn.items()):
    for positive in dq[word]:
        temp = [[positive, x] for x in val[:50]]
        sp += temp

n = len(sp)

r = list(model.predict(sp[:int(0.25 * n)], show_progress_bar=True, batch_size=64))
with open(f'/home/ubuntu/checkpoints/models_checkpoints/entity_search/hard_negatives/query_false_negatives_hn2_1.pkl',
          'wb') as f:
    pickle.dump(r, f)

del r
r = list(model.predict(sp[int(0.25 * n):int(0.5 * n)], show_progress_bar=True, batch_size=64))
with open(f'/home/ubu'
          f'ntu/checkpoints/models_checkpoints/entity_search/hard_negatives/query_false_negatives_hn2_2.pkl',
          'wb') as f:
    pickle.dump(r, f)

del r
r = list(model.predict(sp[int(0.5 * n):int(0.75 * n)], show_progress_bar=True, batch_size=64))
with open(f'/home/ubuntu/checkpoints/models_checkpoints/entity_search/hard_negatives/query_false_negatives_hn2_3.pkl',
          'wb') as f:
    pickle.dump(r, f)

del r
r = list(model.predict(sp[int(0.75 * n):], show_progress_bar=True, batch_size=64))
with open(f'/home/ubuntu/checkpoints/models_checkpoints/entity_search/hard_negatives/query_false_negatives_hn2_4.pkl',
          'wb') as f:
    pickle.dump(r, f)
